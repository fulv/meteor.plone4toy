...
ConnectionsListController = ContentItemListController.extend({
    ...
    connectionInstances: function() {
        var registry = Registry.findOne();
        if (registry) {
            return registry.connections.map(addEvenOdd);
        }
    },
    data: function() {
...
NewConnectionsListController = ContentItemListController.extend({
    subscriptions: function() {
        this.connectionsSub = Meteor.subscribe('connections');
    },
    connectionInstances: function() {
    ...
    data: function() {
        var data = NewConnectionsListController.__super__.data.call(this);
...
Router.route('/connections', {
    ...
    controller: NewConnectionsListController
});
